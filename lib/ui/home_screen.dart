                ...
                TextButton(
                  ...
                  onPressed: () async {
                    ...
                    if (selection != null) {
                      ...
                      try {
                        image = resolveTapeImage((
                          bytes,
                          filePath,
                        ), model: getIt<SettingsService>().zxModel);
                      } catch (_) {}
                      final supported =
                          image != null &&
                          await isResolvedTapeImageSupported(image);
                      if (!context.mounted) return;
                      if (supported) {
                        Navigator.pushNamed(
                          context,
                          PlayerScreen.routeName,
                          arguments: PlayerArgs(filePath, isRemote: false),
                        );
                      } else {
                        var message = tr('invalid_file_format').format([
                          getIt<SettingsService>().zxModel.localTapeExtensions
                              .map((e) => '.${e.toUpperCase()}')
                              .join(', '),
                        ]);
